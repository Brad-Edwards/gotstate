...
def complex_hfsm():
    ...
    machine._graph.set_initial_state(root, subA)
    machine._graph.set_initial_state(subA, a1)
    machine._graph.set_initial_state(subB, b1)
    machine._graph.set_initial_state(subB2, b2a)
...
def test_complex_hfsm_history_capture(complex_hfsm):
    ...
    # Record history for SubA when we're in A2
    root = None  # Root is the state with no parent
    for state in complex_hfsm._graph.get_all_states():
        if complex_hfsm._graph.get_parent(state) is None:
            root = state
            break
    assert root is not None, "Could not find root state"

    subA = complex_hfsm._graph.get_initial_state(root)  # Get SubA from Root's initial state
    complex_hfsm._graph.record_history(subA, complex_hfsm.current_state)
    ...
    # Get history state from graph
    hist = complex_hfsm._graph.get_history_state(subA)
    assert (
        hist is not None
    ), "No history was recorded for SubA. HFSM specs often require storing the last sub-state on exit."
...
def test_complex_hfsm_validation_errors():
    ...
    machine._graph.set_initial_state(root, subX)  # but subX itself is an empty composite
