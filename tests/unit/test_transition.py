...
from gotstate.core.state import ChoiceState, CompositeState, PseudoState, State, StateType
...
class TestTransition(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.event = Event(event_id="test_event", kind=EventKind.SIGNAL, priority=EventPriority.NORMAL)
    ...
    def test_transition_validation(self):
        ...
        with self.assertRaises(ValueError):
            Transition(source=None, target=self.target, event=self.event)

        # Test missing target for non-internal transition
        with self.assertRaises(ValueError):
            Transition(source=self.source, target=None, event=self.event, kind=TransitionKind.EXTERNAL)

        # Test invalid transition kind
        with self.assertRaises(ValueError):
            Transition(source=self.source, target=self.target, event=self.event, kind="invalid")

    def test_transition_guard_evaluation(self):
        """Test transition guard condition evaluation."""
        transition = Transition(source=self.source, target=self.target, event=self.event, guard=self.guard)
    ...
    def test_transition_effect_execution(self):
        ...
        transition = Transition(source=self.source, target=self.target, event=self.event, effect=self.effect)
    ...
    def test_transition_execution(self):
        ...
        transition = Transition(
            source=self.source, target=self.target, event=self.event, guard=self.guard, effect=self.effect
        )
    ...
    def test_internal_transition(self):
        ...
        transition = Transition(
            source=self.source, target=None, event=self.event, effect=self.effect, kind=TransitionKind.INTERNAL
        )
    ...
    def test_transition_priority(self):
        ...
        high_priority = Transition(
            source=self.source, target=self.target, event=self.event, priority=TransitionPriority.HIGH
        )

        normal_priority = Transition(
            source=self.source, target=self.target, event=self.event, priority=TransitionPriority.NORMAL
        )

        low_priority = Transition(
            source=self.source, target=self.target, event=self.event, priority=TransitionPriority.LOW
        )
    ...
    def test_transition_to_pseudostate(self):
        ...
        pseudo_target = PseudoState(state_id="pseudo_target", state_type=StateType.CHOICE, parent=self.source)

        transition = Transition(source=self.source, target=pseudo_target, event=self.event)
...
class TestExternalTransition(unittest.TestCase):
    ...
    def test_external_transition_composite_states(self):
        ...
        transition = ExternalTransition(source=source, target=target, event=self.event)
...
class TestInternalTransition(unittest.TestCase):
    ...
    def test_internal_transition_execution(self):
        ...
        transition = InternalTransition(source=self.state, event=self.event, effect=self.effect)
    ...
    def test_internal_transition_validation(self):
        ...
        with self.assertRaises(ValueError):
            InternalTransition(
                source=self.state, target=State(state_id="other", state_type=StateType.SIMPLE), event=self.event
            )
...
class TestLocalTransition(unittest.TestCase):
    ...
    def test_local_transition_execution(self):
        ...
        transition = LocalTransition(source=self.source, target=self.target, event=self.event, effect=self.effect)
    ...
    def test_local_transition_validation(self):
        ...
        with self.assertRaises(ValueError):
            LocalTransition(source=self.source, target=other_target, event=self.event)
...
class TestCompoundTransition(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures before each test method."""
        self.parent = State(state_id="parent", state_type=StateType.COMPOSITE)
        self.source = State(state_id="source", state_type=StateType.SIMPLE, parent=self.parent)
    ...
    def test_compound_transition_execution(self):
        ...
        segment1 = Transition(source=self.source, target=self.choice, event=self.event)

        segment2 = Transition(source=self.choice, target=self.target, guard=lambda data: True)

        transition = CompoundTransition(segments=[segment1, segment2], event=self.event)
...
class TestProtocolTransition(unittest.TestCase):
    ...
    def test_protocol_transition_execution(self):
        ...
        transition = ProtocolTransition(source=self.source, target=self.target, event=self.event, operation="test_op")
...
class TestTimeTransition(unittest.TestCase):
    ...
    def test_time_transition_execution(self):
        ...
        transition = TimeTransition(source=self.source, target=self.target, event=self.event)
    ...
    def test_time_transition_validation(self):
        ...
        with self.assertRaises(ValueError):
            TimeTransition(source=self.source, target=self.target, event=invalid_event)
...
class TestChangeTransition(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.event = Event(
            ...
            priority=EventPriority.NORMAL,
            data={"condition": "value_changed", "target": "test_var", "old_value": 1, "new_value": 2},
        )

    def test_change_transition_execution(self):
        """Test change transition execution."""
        transition = ChangeTransition(
            source=self.source, target=self.target, event=self.event, condition=lambda old, new: new > old
        )
