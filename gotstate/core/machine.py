...
from gotstate.core.event import Event, EventQueue, EventKind
...
class ProtocolMachine(BasicStateMachine):
    ...
    def add_protocol_rule(self, rule: Dict[str, Any]) -> None:
        ...
        with self._rule_lock:
            ...
            for existing_rule in self._protocol_rules:
                if (existing_rule["operation"] == rule["operation"] and
    ...
    def _apply_operation(self, operation: str) -> None:
        """Apply an operation, updating the current state.
        ...
        with self._rule_lock:
            for rule in self._protocol_rules:
                if (rule["operation"] == operation and
                    ...
                    if rule.get("guard") and not rule["guard"]():
                        raise ValueError(f"Guard condition failed for operation: {operation}")
...
class SubmachineMachine(BasicStateMachine):
    ...
    def add_parent_reference(self, parent: StateMachine) -> None:
        ...
        Raises:
            ValueError: If parent is None or already referenced
        """
        if parent is None:
    ...
    def initialize(self) -> None:
        ...
        Raises:
            ValueError: If initialization fails due to validation errors
            RuntimeError: If initialization fails due to other errors
        """
        with self._status_lock:
            if self._status != MachineStatus.UNINITIALIZED:
                raise ValueError("Machine must be in UNINITIALIZED status")
            ...
            try:
                ...
                # Leave status as INITIALIZING for activate() to complete

            except ValueError as e:
                # Clean up and re-raise ValueError directly
                self._cleanup_components()
                self._status = MachineStatus.UNINITIALIZED
                raise e
            except Exception as e:
                # Clean up and wrap other exceptions
                self._cleanup_components()
                self._status = MachineStatus.UNINITIALIZED
                raise RuntimeError(f"Initialization failed: {e}")
    
    def activate(self) -> None:
        """Activate the submachine.
        ...
        with self._status_lock:
            ...
            try:
                # Start components and event processing
                self._start_components()
                self._start_event_processing()
...
class MachineBuilder:
    ...
    def set_machine_type(self, machine_type: type) -> None:
    ...
    def add_dependency(self, dependent: str, dependency: str) -> None:
        """Add a dependency between component types.
