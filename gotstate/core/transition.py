...
from gotstate.core.types import EffectFunction, GuardFunction, StateData, StateType, TransitionKind, TransitionPriority

if TYPE_CHECKING:
    from gotstate.core.state import ChoiceState, CompositeState, Region, State
else:
    # Forward references for runtime
    from gotstate.core.state import CompositeState, State
...
class GuardCondition:
    ...
    def __and__(self, other: "GuardCondition") -> "GuardCondition":
        ...
        """
        return GuardCondition(lambda data: self.evaluate(data) and other.evaluate(data))
    ...
    def __or__(self, other: "GuardCondition") -> "GuardCondition":
        ...
        """
        return GuardCondition(lambda data: self.evaluate(data) or other.evaluate(data))
    ...
    def __invert__(self) -> "GuardCondition":
        ...
        """
        return GuardCondition(lambda data: not self.evaluate(data))
...
class TransitionEffect:
    ...
    def __add__(self, other: "TransitionEffect") -> "TransitionEffect":
        ...
        """
        return TransitionEffect(lambda data: (self.execute(data), other.execute(data)))
...
class ExternalTransition(Transition):
    ...
    def execute(self) -> bool:
        ...
        if isinstance(self.target, CompositeState):
            for region in self.target.regions.values():
                region.enter()
